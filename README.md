...
ECT identifies a class of perturbations bounded within the chi-squared innovation gate that are statistically indistinguishable from legitimate sensor noise at the single-epoch level, yet accumulate systematically across successive filter updates to corrupt the state estimate beyond operational tolerance. The resulting condition — where the filter reports high confidence in a position estimate that has in fact become operationally meaningless — is termed **Estimator Collapse**. The mission-level consequence is a **Stochastic Mission Kill (SMK)**.
...
<div align="center">
  <img src="Figures/Figure_4.png" width="850" alt="Figure 4">
...
| NIS Gate Compliance | **94.8%** (statistically undetectable by conventional monitors) |
| Filter-reported CEP | **2.43 m** (effectively invariant; $|\Delta| < 0.01$ m from nominal) |
...
> [!IMPORTANT]
> **Stochastic Mission Kill ($MKI = 1.46$) is confirmed for terminal-phase precision requirements where $R_L \leq 3$ m**. The lower $MKI$ observed at $R_L = 15$ m ($MKI = 0.29$) serves only to illustrate that the error has not yet breached a wide-tolerance boundary; it does not indicate that the estimator is healthy or consistent. The 100% $\Gamma(t)$ exceedance confirms the filter has fundamentally lost its ability to bound its own error.

<div align="center">
  <img src="Figures/Figure_3.png" width="850" alt="Figure 3">
...
<div align="center">
  <img src="Figures/Figure_5.png" width="850" alt="Figure 5">
...
### Estimator Instability Number $\Gamma(t)$
...
| $\eta_{info}$ | Uncertainty generation efficiency per unit delivered energy | Analytically estimated (v2.1) |
| $R_{IE}$ | Economic cost–benefit ratio of guidance degradation | Reserved for Phase II HWIL |
...
where $N_{sensors}$ is sensor modality count, $f_{update}$ is filter update frequency (Hz), and $R_{hardening}$ is a normalised electromagnetic shielding coefficient (up to 3.0).
...
$V_i$ values are first-order theoretical estimates intended as empirical vulnerability proxies.
...
├── ECT_3D_Simulation_v2_1.py  # Core Monte Carlo simulation engine (v2.1.0)
├── Figures/
...
@software{barua2026ect,
  ...
  url       = {[https://doi.org/10.5281/zenodo.20132174](https://doi.org/10.5281/zenodo.20132174)}
}
